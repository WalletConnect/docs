...
Every account has two environments: **test mode**, where payments are simulated and no funds move, and **live mode**, where payments execute on-chain for real. Both use the same API — the only difference is which API key you use (`wcp_test_…` vs `wcp_…`). This guide runs through the flow in **test mode first**, so you can build and verify everything safely. See [Test mode](/payments/merchant/test-mode) for the full picture.
