...
Throughout these docs, <span style={{color: "#7C3AED", fontWeight: 600}}>purple</span> indicates test mode. In the dashboard, the environment selector at the top shows which environment you're working in.
...
Test payments don't move through their lifecycle on their own — you drive them. In the test environment, the payments table shows a **Simulation actions** column: open the menu on a payment row and pick **Mark as [status]** to transition it.
