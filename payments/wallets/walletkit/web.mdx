...
After the user selects an option — and **before** fetching required actions — check whether the selected option requires data collection. If it does, the data must be captured first, otherwise fetching the required actions will fail.
...
7. On `IC_COMPLETE`, proceed to `getRequiredPaymentActions()` and then `confirmPayment()` **without** passing `collectedData` — the iframe submits data directly to the backend
...
class PaymentManager {
  ...
  async processPayment(paymentLink) {
    ...
    try {
      ...
      // Step 3: Collect data via iframe if required.
