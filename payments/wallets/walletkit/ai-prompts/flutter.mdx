...
1. **WalletKit SDK integrated** (`reown_walletkit: ^1.5.1` package or newer, which bundles `walletconnect_pay` 1.1.0)
...
6. **Confirm Payment**: Submit the wallet RPC results (`data`) to complete the transaction
...
| `ConfirmPaymentRequest` | Request to confirm payment with the wallet RPC results (`data`) |
...
# pubspec.yaml
dependencies:
  reown_walletkit: ^1.5.1  # Check for latest version
```
...
class PaymentOption {
  ...
  final CollectDataAction? collectData; // Per-option data collection (null if not required)
}
```

### Step 5: Handle Data Collection via WebView

If the selected option's `collectData` is not null and has a `url`, display the URL in a WebView for data collection. The hosted form handles rendering, validation, and T&C acceptance.
...
```dart
/// Sign all actions and return the wallet RPC results, in action order
Future<List<Object>> signActions(List<Action> actions) async {
...
Submit the wallet RPC results (`data`) to complete the payment. Each element of `data` is either a plain `String` (a signature or transaction hash) or, for chains like Tron, a `Map<String, dynamic>` JSON object; the SDK forwards each element verbatim. See [Tron support](/payments/wallets/token-chain-support/tron-support) for the Tron payload.

```dart
/// Confirm the payment with the wallet RPC results
...
**CRITICAL**: The `data` list must match the actions array order exactly. Misalignment causes payment failures.

<Warning>
The `signatures` parameter is deprecated in favor of `data` and will be removed in a future major version. Existing integrations that pass `signatures` keep working; the SDK uses it as a fallback when `data` is null.
</Warning>
...
Future<void> processPayment(String paymentLink) async {
  try {
    ...
    final data = await signActions(actions);
...
### 2. Result Order
**CRITICAL**: `data` must be in the same order as actions.

```dart
// CORRECT
final data = <Object>[];
for (final action in actions) {
  data.add(await signAction(action));
}
...
- [ ] `data` order matches action order
