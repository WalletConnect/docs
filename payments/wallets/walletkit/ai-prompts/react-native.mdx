...
- `@reown/walletkit` 1.6.0 or later integrated and working (bundles `@walletconnect/pay` 1.1.0)
- `@walletconnect/react-native-compat` 2.25.0 or later installed (ships the native Yttrium module: yttrium-wcpay 0.10.60 on Android, YttriumWrapper 0.10.59 on iOS)
...
```json
{
  "@reown/walletkit": "1.6.0",
  "@walletconnect/react-native-compat": "2.25.0"
}
```

These are the minimum versions that include Pay support via `walletKit.pay` with the `data` parameter on `confirmPayment`.
...
**IMPORTANT - ORDERING**: After the user selects an option, check whether that option requires Information Capture. If `selectedOption.collectData?.url` is present, you MUST collect the data via the WebView **before** calling `getRequiredPaymentActions`. The WebView submits the captured data directly to the backend, and `getRequiredPaymentActions` (which triggers `POST /payment/{id}/fetch`) now requires any IC to already be submitted. Do not fetch actions or sign until the WebView signals completion.

If the selected option has no `collectData?.url`, skip this step and go straight to fetching the required actions.

```typescript
// Called when the user selects a payment option.
// Runs the WebView IC step (if needed) BEFORE fetching the required actions.
const onOptionSelected = async (option: PaymentOption) => {
  setSelectedOption(option);

  if (option.collectData?.url) {
    // Step into the WebView; fetchPaymentActions runs in its onComplete.
    setWebViewUrl(option.collectData.url);
    setStep('webviewDataCollection');
    return;
  }

  // No Information Capture needed - fetch actions directly.
  await fetchPaymentActions(option);
};

// Called by the WebView's onComplete callback once IC is submitted to the backend.
const onDataCollectionComplete = async () => {
  if (selectedOption) {
    await fetchPaymentActions(selectedOption);
  }
};
```

See [section 3.6](#3-6-webview-data-collection) for the `PayDataCollectionWebView` component implementation.
...
const fetchPaymentActions = async (option: PaymentOption) => {
  ...
  // Triggers POST /payment/{id}/fetch on the backend.
  // Any required Information Capture must already be submitted via the WebView.
...
Collect each wallet RPC result into `data`, in the same order as the actions. Each element is a plain string (a signature or transaction hash) or, for chains like Tron, a JSON object that the SDK forwards verbatim — see [Tron support](/payments/wallets/token-chain-support/tron-support) for the Tron payload.
...
<Warning>
The `signatures` parameter is deprecated in favor of `data` and will be removed in a future major version. Existing integrations that pass `signatures` keep working; the SDK uses it as a fallback when `data` is omitted.
</Warning>
...
LOADING → ERROR (if failed)
        → CONFIRM (show options) → WEBVIEW (if selected option needs IC) → GET ACTIONS (getRequiredPaymentActions) → CONFIRMING (sign + confirmPayment) → SUCCESS
        → CONFIRM (show options) → GET ACTIONS (getRequiredPaymentActions) → CONFIRMING (sign + confirmPayment) → SUCCESS
```
...
await walletKit.pay.confirmPayment({
  paymentId: string,
  optionId: string,
  data: (string | object)[],       // Wallet RPC results in action order: strings or JSON objects (e.g. Tron)
  // signatures: string[]          // Deprecated alias for `data`, still accepted
});
```

The SDK types `data` as `(string | Record<string, unknown> | unknown[])[]`. A value typed with a TypeScript `interface` does not satisfy `Record<string, unknown>`; declare it as a `type` alias or spread it into an object literal.
...
When `selectedOption.collectData?.url` is present, display the URL in a WebView. This happens immediately after the user selects an option and **before** `getRequiredPaymentActions` is called (see [section 3.2](#3-2-collect-user-data-via-webview-information-capture)). The URL comes from the selected option's `collectData.url`, which is already scoped to that option's account. The hosted form handles rendering, validation, and T&C acceptance.

Wire `onComplete` to your `onDataCollectionComplete` handler (which then calls `fetchPaymentActions`), so action fetching only happens once the WebView has submitted the Information Capture data to the backend.

The form URL accepts optional query parameters — append them to `selectedOption.collectData.url` before loading, preserving existing query parameters:
...
- `theme` — `light` or `dark`, to match your wallet's color mode.
- `themeVariables` — a base64url string exported from the [WalletConnect Pay Dashboard](https://dashboard.walletconnect.com) that overrides design tokens (font, font size, some colors, button/input border radius). Append it verbatim.
