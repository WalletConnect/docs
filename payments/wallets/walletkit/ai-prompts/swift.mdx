...
4. **Result Order Critical** - The `data` array passed to `confirmPayment` must match the actions array order exactly.
...
- iOS 13.0+
- Swift 5.7+
- WalletKit (reown-swift 2.4.0 or later) already integrated (ships YttriumWrapper 0.10.59; required for the `data` parameter in `confirmPayment`)
- WalletConnect Cloud `PROJECT_ID` ([Get one](https://cloud.walletconnect.com))

```swift
// Package.swift
dependencies: [
    .package(url: "https://github.com/reown-com/reown-swift", from: "2.4.0")
]
```
...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  └────┬─────┘                                                    └────┬─────┘
       ...
       │                                                               │
       │  4. Collect user data via WebView (if required)               │
       │                                                               │
       │  5. getRequiredPaymentActions(paymentId, optionId)            │
       │──────────────────────▶┌─────────────┐                         │
       │◀──────────────────────│  Pay API    │                         │
       │  Actions[] (walletRpc — method varies)                             │
       │                       └─────────────┘                         │
       │                                                               │
       │  6. Sign typed data (wallet's EIP-712 signer)                 │
       │                                                               │
       │  7. confirmPayment(data)                                      │
       │──────────────────────▶┌─────────────┐     ┌────────────┐      │
       │                       │  Pay API    │────▶│ Blockchain │      │
       │◀──────────────────────└─────────────┘     └────────────┘      │
...
**CRITICAL**: Keep `data` in the same order as the actions array. Each element of `data` is either a plain string (a signature or transaction hash) or, for chains like Tron, a JSON-encoded object; WalletConnect Pay forwards each element verbatim to the gateway. See [Tron support](/payments/wallets/token-chain-support/tron-support) for the Tron payload.

<Warning>
The `signatures:` overload of `confirmPayment` is deprecated in favor of `data:` and will be removed in a future major version. Existing integrations that pass `signatures:` keep working; the overload forwards its value to `data`.
</Warning>
...
When `selectedOption.collectData.url` is present, display the URL in a `WKWebView`. The hosted form handles rendering, validation, and T&C acceptance.
...
func buildFormUrl(
    ...
    if let theme = theme { queryItems.append(URLQueryItem(name: "theme", value: theme)) }
...
// Confirm payment with the wallet RPC results (plain strings or JSON-encoded objects, in action order)
...
// Deprecated: `signatures:` is an alias for `data:`; still compiles and forwards to `data`
WalletKit.instance.Pay.confirmPayment(
    paymentId: String,
    optionId: String,
    signatures: [String],
...
| `invalidSignature` | Signature mismatch | Check `data` order matches actions |
...
2. **Wrong result order** - The `data` array must match the actions array order exactly.
