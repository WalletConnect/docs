...
- iOS 13.0+
- Android API 23+
- ReownWalletKit (`reown_walletkit`) 1.5.1 or later (bundles `walletconnect_pay` 1.1.0)
...
for (final action in actions) {
  ...
  switch (rpc.method) {
    case 'eth_signTypedData_v4':
      data.add(await signTypedData(rpc.chainId, rpc.params));
      break;
    case 'eth_sendTransaction':
      data.add(await sendTransaction(rpc.chainId, rpc.params));
      break;
    case 'personal_sign':
      data.add(await personalSign(rpc.chainId, rpc.params));
      break;
    default:
...
final confirmResponse = await walletKit.confirmPayment(
  request: ConfirmPaymentRequest(
    paymentId: 'payment-id',
    optionId: 'option-id',
    data: data, // Wallet RPC results from the Sign Actions step
...
class PaymentService {
  ...
  Future<void> processPayment(String paymentLink) async {
    try {
      ...
      // Step 5: Execute wallet actions and collect the results
      final data = <Object>[];
      for (final action in actions) {
        ...
        data.add(await signAction(action.walletRpc));
      }
...
ConfirmPaymentRequest({
  ...
  List<Object>? data,                   // Wallet RPC results in action order: Strings or Map<String, dynamic> JSON objects
  @Default([]) List<String> signatures, // Deprecated alias for `data`, still accepted as fallback when data is null
  int? maxPollMs,
})
```
...
5. **Result Order**: Keep `data` in the same order as the actions array
