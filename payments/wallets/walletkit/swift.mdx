...
If the selected option has `collectData` set, you must collect user information **before** fetching the required actions. The backend requires Information Capture to be completed first and will reject `getRequiredPaymentActions` otherwise:
...
<Note>
The WebView submits the captured data directly to the backend. You do **not** pass `collectedData` to `confirmPayment` when using the WebView approach.
</Note>
