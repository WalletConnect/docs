...
- **Target SDK**: 36
- **JVM Target**: 11
- **WalletConnect Pay SDK**: 1.3.0+ (`com.walletconnect:pay`, ships yttrium-wcpay 0.10.60). Required for the `data` parameter in `confirmPayment`.
...
```kotlin
dependencies {
    implementation("com.walletconnect:pay:1.3.0")
}
```
...
| `confirmPayment(paymentId, optionId, data = ..., collectedData = null)` | Confirm and finalize payment. `data: List<String>` holds the wallet RPC results in action order (plain strings or JSON-encoded objects). `collectedData` is the existing optional parameter for collected data fields — unchanged |
| `confirmPayment(paymentId, optionId, signatures = ...)` | Deprecated alias for `data`; still accepted |
