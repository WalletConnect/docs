...
| `confirmPayment(paymentId, optionId, data, collectedData)` | Confirm and finalize payment. `data: List<String>` holds the wallet RPC results in action order (plain strings or JSON-encoded objects) |
