...
- Swift 5.7+
- Xcode 14.0+
- WalletConnectPay 2.4.0 or later (ships YttriumWrapper 0.10.59). Required for the `data` parameter in `confirmPayment`.
...
| `confirmPayment(paymentId:optionId:data:collectedData:maxPollMs:)` | Confirm and execute the payment. `data: [String]` holds the wallet RPC results in action order (plain strings or JSON-encoded objects) |
| `confirmPayment(paymentId:optionId:signatures:collectedData:maxPollMs:)` | Deprecated alias for the `data:` overload; still compiles and forwards to `data` |
...
3. **Result Order**: Keep `data` in the same order as the actions array
