...
<Card title="Sample Wallet - React Native" icon="github" href="https://github.com/reown-com/react-native-examples/tree/main/wallets/rn_cli_wallet">
...
- React Native 0.70+
- `@walletconnect/pay` 1.1.0 or later
- `@walletconnect/react-native-compat` 2.25.0 or later, installed and linked (ships the native Yttrium module: yttrium-wcpay 0.10.60 on Android, YttriumWrapper 0.10.59 on iOS)
...
This SDK requires the WalletConnect React Native native module. Make sure you have `@walletconnect/react-native-compat` 2.25.0 or later installed and linked in your React Native project:
...
sequenceDiagram
    ...
    Wallet->>PaySDK: confirmPayment(data)
...
<Warning>
Keep `data` in the same order as the actions array.
</Warning>
...
Submit the wallet RPC results (`data`) and collected data to complete the payment. Each element of `data` is either a plain string (a signature or transaction hash) or, for chains like Tron, a JSON object; the SDK forwards each element verbatim. See [Tron support](/payments/wallets/token-chain-support/tron-support) for the Tron payload.
...
interface ConfirmPaymentParams {
  ...
  /** Option ID */
  optionId: string;
  /** Wallet RPC results in action order: plain strings or JSON objects (e.g. Tron) */
  data: (string | object)[];
  /** @deprecated Alias for `data`, still accepted */
  signatures?: string[];
}
```
...
4. **Result Order**: Keep `data` in the same order as the actions array
