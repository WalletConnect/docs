...
The simple WalletConnect Pay flow signs an EIP-3009 `transferWithAuthorization` typed-data payload — the gasless transfer used by USDC and similar stablecoins. USDT does not implement EIP-3009, so WalletConnect Pay routes it through Uniswap's [Permit2](https://github.com/Uniswap/permit2) contract instead.
...
Inspect `option.actions` for an `eth_sendTransaction` entry. If one is present, the option is a Permit2 flow and your wallet should prepare for an on-chain step.
