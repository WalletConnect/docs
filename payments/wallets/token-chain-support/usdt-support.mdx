...
sequenceDiagram
    ...
    Wallet->>PaySDK: confirmPayment(data: [txHash, signature])
    PaySDK-->>Wallet: status
```

<Warning>
`data[]` passed to `confirmPayment` must match `actions[]` in **order and length** — the approve tx hash first, then the Permit2 signature. Both results are plain strings; `data` also accepts JSON objects for chains such as [Tron](/payments/wallets/token-chain-support/tron-support). The former `signatures` parameter is deprecated but still accepted.
</Warning>
...
3. Push both results into `data[]` in the order the actions were returned.
4. Call `confirmPayment(data)`.
...
for (action in actions.filterIsInstance<Wallet.Model.RequiredAction.WalletRpc>()) {
    data += when (action.action.method) {
        "eth_sendTransaction" -> {
...
for (const action of actions) {
  ...
  switch (method) {
    ...
    case "eth_signTypedData_v4":
      data.push(await wallet.signTypedData(chainId, parsed));
      break;
    default:
...
for (final action in actions) {
  ...
  switch (rpc.method) {
    case 'eth_sendTransaction':
      ...
      data.add(await service.sendPayTransaction(
        Map<String, dynamic>.from(params),
      ));
      break;
    case 'eth_signTypedData_v4':
      data.add(service.ethSignTypedDataV4(rpc.params));
      break;
    default:
...
<Steps>
<Step title="First-time ERC-20 with Permit2">
...
- Complete successfully with both results in `data[]`
</Step>

<Step title="Repeat ERC-20 payment">
...
<Step title="Native token payment">
Test a payment using the chain's native token (e.g., ETH on Base). Verify that:
- The transaction hash is correctly included in `data[]`
- The payment completes successfully
</Step>

<Step title="Rapid option switching">
...
<Step title="Expired payment handling">
