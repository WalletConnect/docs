...
sequenceDiagram
    ...
    Gateway-->>Wallet: status: processing (accepted into the mempool)
    Chain-->>Gateway: Inclusion
    Gateway-->>Wallet: status: succeeded / failed (via polling)
...
5. **Asynchronous settlement** — Tron accepts a transaction into the mempool before inclusion. A `processing` confirm result is the happy path, not an error.
...
### Treat `processing` as in flight

Tron accepts the transaction into the mempool before it is included in a block. The confirm response reports `processing` while WalletConnect Pay polls for inclusion; the SDK keeps polling until the status is final (`succeeded` or `failed`). Present `processing` as an in-flight payment, not as a failure.
...
- The result shows `processing`, then the payment settles (check the payment in the explorer)
</Step>

<Step title="Unfunded Tron account">
Advertise a Tron account with no TRX or USDT. Verify that no Tron option is offered and that the other options still load normally.
</Step>
