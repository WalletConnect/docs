...
6. **Confirm Payment**: Submit the wallet RPC results (`data`) and complete the payment
...
<Note>
On Solana, payment options for non-native tokens only appear once this address has an associated token account (ATA) for that token. The easiest way to create one is to send a small amount of the token to this address — the transfer covers the SOL needed to create the account. Alternatively, send a create-account transaction and pay for it with SOL.
</Note>
