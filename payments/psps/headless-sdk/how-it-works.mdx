...
flowchart LR
    ...
    Server["Your server route<br/>createWcpClient<br/>(holds the API key)"]
...
`pay-core` exposes two entry points for exactly this: `createHttpTransport` for the browser (talks to *your* server) and `createWcpClient` (imported from `@walletconnect/pay-core/server`) which holds the API key and talks to WCP. In a Next.js app, your Route Handler sits in the middle.
