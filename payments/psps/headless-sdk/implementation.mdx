...
import 'server-only'
import { createWcpClient } from '@walletconnect/pay-core/server'

const client = createWcpClient({
...
1. **A test API key** on your server proxy. Create a `wcp_test_…` key in the [dashboard](https://merchant.pay.walletconnect.com/en/api-keys) and use it as `WCP_WALLET_API_KEY`. Payments created and driven with a test key settle synthetically — nothing runs on-chain (see [Test mode](/payments/test-mode)).
