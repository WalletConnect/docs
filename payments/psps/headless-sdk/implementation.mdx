...
- **Headless checkout access** on your WalletConnect Pay account — request it from **Developers → API Keys** in the [dashboard](https://merchant.pay.walletconnect.com/en/api-keys). See [Request access and collect the Keys](/payments/psps/headless-sdk/overview#request-access-and-collect-the-keys).
- A **Reown project ID**, shown on the **Headless checkout** card once access is granted (public — used for wallet connection).
- A **Gateway wallet key** (prefix `wcw_`), created from the same card. Secret — server-side only.
...
```bash .env.local
# Reown project ID from the Headless checkout card — wallet connection / QR pairing (public)
NEXT_PUBLIC_APPKIT_PROJECT_ID=

# Gateway wallet key (wcw_…) — server-side only, NEVER exposed to the browser
