...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>WCP API client — contract types, CAIP utilities, the browser <code>Transport</code> seam (<code>createHttpTransport</code>), and the server-side <code>createEngineClient</code> that holds your API key. The foundation.</td>
      <td>—</td>
    </tr>
    <tr>
      <td style={{ whiteSpace: "nowrap" }}>
...
`EngineClient` exposes the same five methods as `Transport`, but attaches the secret `Api-Key` and `Wcp-Version` headers and calls WCP directly. Use it only on the server.
...
<CardGroup cols={2}>
  ...
  <Card title="API Reference" icon="code" href="/payments/api-reference/index">
