...
The prompt below contains the complete spec plus a test vector, so an AI coding agent (Claude Code, Cursor, Codex, and the like) can implement and verify the handler in one shot. Copy it as-is:

````text Prompt: implement WalletConnect Pay webhook verification
