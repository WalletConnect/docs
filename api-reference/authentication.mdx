...
Your account has two kinds of keys, one per environment:

| Key prefix | Environment | Behavior |
|---|---|---|
| `wcp_test_` | **Test** | Payments are simulated; no funds move. See [Test mode](/payments/merchant/test-mode). |
| `wcp_` | **Live** | Real payments with on-chain settlement. |

The key is the **only** thing that selects the environment. Both key types work against the same base URL and the same endpoints, and there is no test flag or mode parameter on any request.

<Warning>
Responses and webhook events do not include an environment field. A resource's environment is determined by the key that created it.
</Warning>
