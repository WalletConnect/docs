...
Wallet SDK Link Mode is a low latency mechanism for transporting [One-Click Auth](/wallets/react-native/one-click-auth) requests and session requests over Universal Links, reducing the need for a WebSocket connection with the Relay. This significantly enhances the user experience when connecting native dApps to native wallets by reducing the latency associated with network connections, especially when the user has an unstable internet connection.

<Note>
Make sure that [One-Click Auth](/wallets/react-native/one-click-auth) is implemented before enabling Link Mode. 
</Note>
